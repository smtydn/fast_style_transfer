...
def loss_network(width=256, height=256, bs=32):
    ...
    total_variation_loss = layers.Lambda(utils.get_total_var_loss, output_shape=(1,), arguments={'width': width, 'height': height}, name='tv')([inputs])
...
def training_model(width=256, height=256, bs=32):
    inputs = layers.Input(shape=(width, height, 3), dtype='float32')
    ...
    # scale output from [-1, 1] to [0, 255]
    outputs = layers.Lambda(lambda x: (x + 1) * 127.5, name='output')(relu_db3)
